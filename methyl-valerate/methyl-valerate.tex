...
\begin{figure}[htb]
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \resizebox{\linewidth}{!}{\input{figures/pressure-traces.pgf}}
        \caption{Selected pressure traces around the NTC region of ignition delay for $\phi=1.0$}
        \label{fig:pressure-traces}
    \end{minipage}\hfill%
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \resizebox{\linewidth}{!}{\input{figures/simulation-comparison.pgf}}
...
To try and understand the underlying reasons for the disagreement between the
\textcite{Dievart2013} model and the data, we constructed an alternate model
...
ignition delay, but still predicts an NTC region where none is present in the
experimental data.
...
studies are required to ensure that the RMG model includes the relevant reaction
pathways.
...
further detailed studies of the models.
...
particular radical by all the reactions that can form that radical. The columns
may not sum to \SI{100}{\percent} due to independent rounding.

At the relatively low temperature and high pressure of this analysis, all of the
fuel is destroyed by \ce{H}-atom abstractions to form the fuel radicals shown.
It can be seen that the two models have quite different distributions of
products from the first \ce{H}-abstraction reactions. The model of
\textcite{Dievart2013} predicts that \ce{H}-abstraction from the second carbon
will be the most prevalant, followed closely by abstraction from the methyl
group. This is in line with the bond energy of the radicals; we expect that the
presence of the oxygen atoms on the right side of the molecule will cause
radicals formed at those carbons to be favored. However, the RMG model predicts
that radicals in the middle of the carbon chain will be primarily formed. The
cause of this discrepancy is under investigation, but it may be caused by the
estimation of thermodynamic properties of the radicals.
...
\begin{figure}
    ...
    \begin{minipage}[t][][b]{0.68\textwidth}
        \centering
        \captionof{table}{Percent of MV destroyed to form radical species with a hydrogen atom missing at the location indicated in the first column}
        \label{tab:mv-radicals}
        \begin{tabular}{cSS}
            \toprule
            Radical & {\textcite{Dievart2013} [\si{\percent}]} & {RMG Model [\si{\percent}]} \\
            \midrule
            2 & 29.2 & 7.4 \\
            3 & 17.5 & 36.0 \\
            4 & 17.5 & 41.1 \\
...
reactions than the literature model, it is still unable to predict the ignition
delays satisfactorily. Possible reasons for the discrepancy include missing
reaction pathywas, incorrect rate estimates, and incorrect thermodynamic
property estimates. Future work will include investigation of the discrepancies
between models and experiments to further understand the kinetics of methyl
valerate autoignition.

\section{Acknowledgements}\label{sec:acknowledgements}

This paper is based on material supported by...
